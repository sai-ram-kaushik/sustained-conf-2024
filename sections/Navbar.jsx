        ...
        <div className='flex items-center justify-between w-full h-full px-6 lg:px-0'>
          ...
          <div className='hidden lg:flex items-center gap-5'>
            {menuList.map((list, index) => (
              <ul key={index} className='text-lg font-normal hover:text-secondary duration-200 cursor-pointer'>
                <Link href={list.path}> <li>{list.label}</li></Link>
              </ul>
            ))}
          </div>

          <div className='block lg:hidden p-3 rounded-full bg-secondary text-background' onClick={handleChange}>
